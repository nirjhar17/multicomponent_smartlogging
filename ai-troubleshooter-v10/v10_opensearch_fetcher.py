...
class OpenSearchLogFetcher:
    ...
    def fetch_logs(
        ...
        index_patterns = {
            ...
            'server': 'server-*',
            'database': 'database-*',
            'kubernetes': 'openshift-logs',  # Updated to actual index name
            'firewall': 'firewall-*'
        }
        ...
        if component_type == 'kubernetes':
            # For Kubernetes, we need to fetch both application logs AND events
            if namespace and pod_name:
                # Query for BOTH: application logs from the pod AND EventRouter logs for the pod
                should_clauses = [
                    ...
                    {'bool': {
                        'must': [
                            ...
                            {'bool': {'must_not': [{'term': {'kubernetes.container_name.keyword': 'kube-eventrouter'}}]}}
                        ]
                    }},
                    # Events from EventRouter for this specific pod (stored in old_event.involvedObject.name)
                    {'bool': {
                        'must': [
                ...
                # Just namespace filter (get all logs from namespace, including events)
                should_clauses = [
                    # Application logs from namespace
                    {'bool': {
                        'must': [
                            {'term': {'kubernetes.namespace_name.keyword': namespace}},
                            {'bool': {'must_not': [{'term': {'kubernetes.container_name.keyword': 'kube-eventrouter'}}]}}
        ...
        try:
            ...
            for hit in hits:
                ...
                if component_type == 'kubernetes':
                    # Check if this is an event from EventRouter (has old_event field)

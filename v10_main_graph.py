"""
AI Troubleshooter v10 - Main LangGraph Workflow
Multi-Agent Self-Corrective RAG System with Multiple Model Support
"""
...
from v10_graph_nodes import Nodes, get_nodes_instance
from v10_graph_edges import Edge
...
def create_workflow(
    ...
    """
    Create the LangGraph workflow for log analysis with specified AI model.
    
    Args:
        model_config: Configuration for the AI model to use for generation
        llama_stack_url: URL to Llama Stack service (for embeddings)
        max_iterations: Maximum self-correction iterations (default: 3)
        vector_db_id: Vector database ID (default: from env or openshift-logs-v10)
        reranker_url: BGE Reranker URL (default: from env)
        checkpointer: Optional checkpointer for conversation memory (MemorySaver or PostgresSaver)
        
    Returns:
        Compiled StateGraph application
    """
    print("🏗️  Building Multi-Agent Log Analysis Workflow...")
    print(f"   🤖 Using AI Model: {model_config.display_name}")
    
    # Initialize nodes with selected model
    ...
    nodes = get_nodes_instance(model_config, llama_stack_url, vector_db_id, reranker_url)
    ...
    if checkpointer:
        ...
        print("   💾 Checkpointer enabled for conversation memory!")
    else:
        app = workflow.compile()
        print("   ⚠️  No checkpointer - conversations won't persist")

#!/usr/bin/env python3
"""
AI Troubleshooter v10 - OpenSearch Multi-Component Analysis with LangSmith
Multi-Agent RAG with OpenSearch for analyzing logs from multiple infrastructure components
🆕 Now with LangSmith tracing and observability!
"""
...
# Add v10 modules to path
sys.path.insert(0, os.path.dirname(__file__))

# Import v10 multi-agent components
try:
    from v10_main_graph import create_workflow, run_analysis
    ...
    from v10_opensearch_fetcher import OpenSearchLogFetcher, create_opensearch_fetcher
except ImportError as e:
    st.error(f"Failed to import required modules: {e}")
    st.error("Make sure all v10 modules are present and OpenSearch fetcher is available.")
    st.stop()
...
VECTOR_DB_ID = os.getenv("VECTOR_DB_ID", "openshift-logs-v10")
...
if 'selected_model_id' not in st.session_state:
    st.session_state.selected_model_id = "llama-3b"  # Default to local Llama 3B

if 'available_models' not in st.session_state:
    from v10_model_config import get_available_models
    st.session_state.available_models = get_available_models()
    print(f"✅ Found {len(st.session_state.available_models)} available models")

if 'needs_workflow_update' not in st.session_state:
    st.session_state.needs_workflow_update = False

# Initialize workflow with checkpointer and selected model
if 'workflow' not in st.session_state or st.session_state.needs_workflow_update:
    from v10_model_config import MODELS
    
    # Get selected model config
    selected_model_config = MODELS.get(st.session_state.selected_model_id)
    if not selected_model_config:
        # Fallback to default if selected model not found
        st.session_state.selected_model_id = "llama-3b"
        selected_model_config = MODELS["llama-3b"]
    ...
    st.session_state.needs_workflow_update = False
    print(f"✅ Workflow initialized with {selected_model_config.display_name}")
...
<div class="main-header">
    <h1>🔍 AI Troubleshooter v10 - Multi-Component AIOps</h1>
    <p>Analyze logs from Storage, Servers, Databases, Kubernetes & Firewalls | Multi-Agent RAG with OpenSearch + LangSmith</p>
    <p style="font-size: 0.9em; color: #10b981;">🆕 Now with LangSmith tracing & observability!</p>
</div>
""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    # Show active model
    if 'available_models' in st.session_state and st.session_state.selected_model_id in st.session_state.available_models:
        current_model = st.session_state.available_models[st.session_state.selected_model_id]
        st.info(f"🤖 **Active Model**: {current_model.display_name}")
    else:
        st.info(f"🤖 **Active Model**: {LLAMA_STACK_MODEL}")
    
    st.info(f"🎯 **BGE Reranker**: Enabled")
    st.success(f"📈 **LangSmith**: Tracing Active")
    
    if langsmith_client:
        st.markdown(f"🔗 [View Traces](https://smith.langchain.com/o/default/projects/p/{LANGSMITH_PROJECT})")
...
# Model selection dropdown (before chat input)
st.markdown("### 🤖 Select AI Model")

available_models = st.session_state.get('available_models', {})

if available_models:
    # Create columns for model selector
    col1, col2 = st.columns([3, 1])
    
    with col1:
        model_options = list(available_models.keys())
        current_index = model_options.index(st.session_state.selected_model_id) if st.session_state.selected_model_id in model_options else 0
        ...
        selected_model_id = st.selectbox(
            ...
            index=current_index,
            key="model_selector",
            help="Select which AI model to use for troubleshooting. Local models are fast and free, foundation models are more capable."
        )
        
        # Update session state if changed
        if selected_model_id != st.session_state.selected_model_id:
            st.session_state.selected_model_id = selected_model_id
            # Recreate workflow with new model
            st.session_state.needs_workflow_update = True
            st.rerun()
    
    with col2:
        # Show model info
        current_model = available_models[st.session_state.selected_model_id]
        with st.expander("ℹ️ Info"):
            st.caption(f"**Provider**: {current_model.provider}")
            st.caption(f"**Description**: {current_model.description}")
else:
    st.error("⚠️ No models available. Check configuration and API keys.")
    ...
    with st.chat_message("assistant"):
        with st.spinner("🤖 Analyzing..."):
            try:
                ...
                if component_type == "kubernetes":
                    ...
                    try:
                        ...
                        if not logs or logs.strip() == "":
                            logs = f"No logs found for pod {pod_name} in namespace {namespace} (last {time_range})"
                            st.warning(f"⚠️ No logs returned from OpenSearch")
                        else:
                            log_lines = logs.count('\n')
                            st.success(f"✅ Retrieved {log_lines} log entries ({len(logs)} chars)")
                            
                    except Exception as e:
                        st.error(f"❌ Error fetching K8s logs: {e}")
                        logs = f"Error fetching logs: {str(e)}"
                    
                    events = ""  # EventRouter logs (can be fetched separately if needed)
                ...
                # Invoke workflow with thread_id for memory and LangSmith tracing!
                ...
                if result and "answer" in result:
                    ...
                    # NEW: Display structured Ansible output if available
                    ansible_output = final_state.get("ansible_output")
                    if ansible_output:
                        try:
                            from v10_ansible_schemas import AnsibleRemediationOutput
                            from v10_eda_integration import EDAIntegration
                            import tempfile
                            
                            # Parse structured output
                            remediation = AnsibleRemediationOutput(**ansible_output)
                            display_data = remediation.to_display_format()
                            
                            st.success("✅ **AI has generated a structured alert!**")
                            
                            # Expandable RCA section
                            with st.expander("🔍 **Root Cause Analysis**", expanded=False):
                                st.markdown(display_data["rca"])
                            
                            # Expandable Structured Alert JSON section
                            with st.expander("📋 **Structured Alert JSON**", expanded=False):
                                st.json(ansible_output)
                            
                            # Action buttons
                            col1, col2, col3 = st.columns(3)
                            
                            with col1:
                                if st.button("📤 Send to EDA", type="primary", key=f"eda_{datetime.now().timestamp()}"):
                                    eda = EDAIntegration()
                                    result = eda.send_to_eda(remediation)
                                    if result["success"]:
                            ...
                            with col2:
                                if st.button("💾 Save RCA", key=f"save_{datetime.now().timestamp()}"):
                                    eda = EDAIntegration()
                                    result = eda.save_to_files(remediation)
                                    if result["success"]:
                                        st.success(result["message"])
                                        st.json({
                                            "RCA": result.get("rca_file", ""),
                                            "Alert": result.get("alert_file", "")
                                        })
                                    else:
                            ...
                            with col3:
                                ...
                                json_bytes = json_lib.dumps(ansible_output, indent=2).encode()
                                st.download_button(
                                    label="📥 Download Structured Output",
                                    data=json_bytes,
                                    file_name=f"alert_{display_data['alert_name']}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
                                    mime="application/json",
                                    key=f"download_{datetime.now().timestamp()}"
                                )
                            
                        except Exception as ansible_ui_error:
                            # Don't break the UI if ansible display fails
                            st.warning(f"⚠️ Could not display structured output: {ansible_ui_error}")
